...
<p:declare-step version="1.0" name="main" type="tapas:validity-report"
  ...
  <p:input port="source" primary="true"/> <!-- TEI input file -->
  <p:input port="grammar"/> <!-- TEI RELAX NG (XML syntax) grammar -->
  <p:input port="rules"/> <!-- TEI ISO Schematron schema [1] -->
  <p:output port="result" primary="true" sequence="true">
    <p:pipe step="take-both" port="result"/>
  </p:output>
  <p:documentation>
    The output is a singe XML document which (I think) should match
    the following declaration:
           element tapas:errors { RNG?, SCH }
           RNG = element c:errors { c:error+ }
           SCH = element svrl:schematron-output { [2] }
    That is, there will be no "c:errors" child unless there were errors found
    by RELAX NG validation; there will be a full SVRL report whether or not
    any assertions or reports generated messages.
    Note: If the RELAX NG validator generates error output based on the embedded
    Schematron in the RELAX NG schema, they are summarily discarded.
  </p:documentation>
  <p:documentation>
    [1] When run in oXygen, duplicate namespace declarations do not seem to be a problem.
        (They are in `probatron`, although the spec does not say they should be.)
        I have not tested in eXist, yet.
    [2] The SCH output is in the Schematron validation report language; see 
        Annex D of ISO 19757-3:2016
  </p:documentation>
